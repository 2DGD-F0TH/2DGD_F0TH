...
\placeholder
<!-- TODO: not a pathfinding algorithm, useful for non-interactive NPCs: 1) if there is only one direction: take it, 2) take a direction at random, if there is a wall, continue your search clockwise 3) You cannot go back (unless it's the only direction), used in RCT2's mazes -->
...
![Example of a behaviour tree](./images/AI/Behaviour_Tree.pdf){width=60%}
