...
float dot_product(Vector2D u, Vector2D v) { return (u.x * v.x) + (u.y * v.y); }

Vector2D scale_vector(float factor, Vector2D v) {
  return Vector2D(x = factor * v.x, y = factor * v.y);
}

float magnitude(Vector2D v) { return sqrt(dot_product(v, v)); }
...
if (collides(obj1, obj2)) {
  ...
  if (s < 0) {
    obj2.velocity = obj2.velocity + scale_vector(factor, unit_ucoll);
    obj1.velocity = obj1.velocity + scale_vector(factor, unit_ucoll);
  }
  // ...
}
// ...
